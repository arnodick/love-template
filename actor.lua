...
local function damage(a,d)
	if not a.delete then
		...
		if flags.get(a.flags,EF.damageable) then
			a.hp = a.hp - d
			if _G[EA[Game.name][a.t]]["damage"] then
				_G[EA[Game.name][a.t]]["damage"](a)
			end
			for i=1,4 do
				actor.make(EA[Game.name].debris,a.x,a.y)
			end
			...
			if a.hp<1 then
				...
				if flags.get(a.flags,EF.explosive) then
					actor.make(EA[Game.name].explosion,a.x,a.y,0,0,EC.white,20*(a.size))
				end

				if flags.get(a.flags,EF.character) then
					character.dead(a)
				end
				if _G[EA[Game.name][a.t]]["dead"] then
					_G[EA[Game.name][a.t]]["dead"](a)
				end
			end
		end
	end
...
local function corpse(a,tw,th,hack)
	...
	local body=actor.make(EA[Game.name].debris,a.x,a.y)
	body.decel=0.1
	if not hack then
		...
		if choice==1 then
			...
			local body2=actor.make(EA[Game.name].debris,a.x,a.y)
			body2.decel=0.1
		...
		local body3=actor.make(EA[Game.name].debris,a.x,a.y)
		body3.decel=0.2
		...
		local body4=actor.make(EA[Game.name].debris,a.x,a.y)
		body4.decel=0.2
