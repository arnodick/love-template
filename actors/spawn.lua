		...
		if l.enemies[spawnnum] then
			...
			if l.actordrops then
				if l.actordrops[spawnnum] then
					module.make(enemy,Enums.modules.drop,l.actordrops[spawnnum])
				end
			end
			...
			if p1 then
				if p1.droppedby==spawnnum then
					module.make(enemy,Enums.modules.drop,"portal",1)
				end
			end
			...
			if p2 then
				if p2.droppedby==spawnnum then
					module.make(enemy,Enums.modules.drop,"portal",2)
				end
			end
			...
			if pstore then
				if pstore.droppedby==spawnnum then
					module.make(enemy,Enums.modules.drop,"portal","store")
				end
			end
		end
