...
local function keyreleased(m,key)
	if key=='z' then
		...
		if m.menu_functions[i] then
			local g=Game
			module.make(g.levels.current,EM.transition,easing.linear,"transition_timer",0,1,240,m.menu_functions[i],m.menu_function_args[i],EM.transitions.screen_transition_blocks)
