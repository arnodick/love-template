local function make(a,angle,len,speed,anglemin,anglemax)
	a.gun={}
	a.gun.angle=angle
	a.gun.leninit=len
	a.gun.len=len
...
local function control(g,gs,a,vx,vy,shoot)
	...
	local dir=vector.direction(vx,vy)
	g.vec[1]=math.cos(dir)
	g.vec[2]=math.sin(dir)
	...
	if g.delta<=0 then
		...
		if shoot then
			local guntype=Guntypes[a.gt]
			sfx.play(guntype.snd)
			g.len=2
			g.heat = g.heat + guntype.heat
			...
			if guntype.proj==4 or guntype.proj==5 then
				...
				for b=1,guntype.num do
					...
					actor.make(Enums.actors.projectile,guntype.proj,g.x,g.y,Enums.colours.pure_white,-g.angle+rand,bvel+math.randomfraction(1),a.t)
				end
